<template>
  <div class="read-blog">
    ...
    <div class="title word-ellipsis">
      {{ readOrEdit.blog.title || "Untitled" }}
    </div>
    <div class="more-info">
  ...
  methods: {
    ...
    lastPage() {
      ...
      let myPos = logList
        ...
        .indexOf(this.readOrEdit.blog.createTime);
      if (myPos - 1 >= 0) {
        EventHub.pageInfo.current = Math.ceil(
          (myPos - 2) / EventHub.pageInfo.pagenum
        );
        this.$router.push({
          name: "blogArticle",
          params: {
            id: logList[myPos - 1].createTime,
            isReadable: true,
            key: logList[myPos - 1].title,
          },
        });
      } else {
    ...
    nextPage() {
      let logList = this.$store.state.logList;
      let myPos = logList.indexOf(this.readOrEdit.blog);
      if (myPos + 1 < logList.length) {
        EventHub.pageInfo.current = Math.ceil(
          (myPos + 2) / EventHub.pageInfo.pagenum
        );
        this.$router.push({
